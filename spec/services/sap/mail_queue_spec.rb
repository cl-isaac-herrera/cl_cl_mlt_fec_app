...
RSpec.describe Sap::MailQueue do
  ...
  before do
    SlResource.create!(code: 'getMailInformation', resource: 'U_CL_FEC_MAILSDETAILS',
                       query_params: '$filter=(U_DocEntry eq @DocEntry and U_DocType eq @DocType and ' \
                                     'U_Status ne 4 and U_Status ne 5)',
                       page_size: 0)
    SlResource.create!(code: 'createMailQueue', resource: 'U_CL_FEC_MAILSDETAILS', page_size: 0)
    SlResource.create!(code: 'updateMailQueue', resource: 'U_CL_FEC_MAILSDETAILS(#Code#)', page_size: 0)
  end
  ...
  describe '#find' do
    it 'resuelve el path del catálogo con DocEntry y DocType sustituidos' do
      ...
      expect(client).to have_received(:get).with(
        "U_CL_FEC_MAILSDETAILS?$filter=(U_DocEntry eq 25 and U_DocType eq '01' and U_Status ne 4 and U_Status ne 5)"
      )
    end
