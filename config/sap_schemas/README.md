...
| `company_config_udt.json` | `@CL_FEC_ISSUERCONFIG` | sí | Configuración del emisor ante Hacienda: razón social, tipo de identificación, código de actividad económica y registro fiscal 8707. Una sola fila por base de SAP (`Code` fijo `1`). La administra `Sap::CompanyConfig`, la sección "Datos Generales" del formulario de compañías (`CLAUDE.md` §32, caso `company_config_udt`). El nombre comercial (`companies.name`) y la cédula (`companies.issuer_id_number`) NO están acá — ver la nota de cabecera del servicio. |
